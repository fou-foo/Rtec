<?php 
 ...
 $query = "SELECT COUNT(*) AS cuenta FROM encuesta WHERE numero='$encuesta'";
 ...
 { 
 	...
 	$totalPreguntas = $fila["preguntas"];//se consulta el numero total de preguntas de la encuesta
 }  else {
 	$totalPreguntas = 0;
 }
 ...
 {		
	#se comienza validacion de si el alumno ha ingresado sus datos personales	
	$query = "SELECT COUNT(*) AS cuenta FROM personales WHERE matricula='$usuario'";
	$resultado = $con->prepare($query);
	$resultado->execute();
	$row = $resultado->fetch();
	if ($row['cuenta'] > 0 ) 
    ...
    #se termina validacion de si el alumno ha ingresado sus datos personales	
    //$query = "SELECT * FROM personales WHERE matricula='$usuario'";
	$encuestaTerminada = 0;
	for($pregunta = 1 ; $pregunta <= $totalPreguntas; $pregunta++) 
	{
		$sql = "SELECT COUNT(*) AS cuenta FROM resultados WHERE alumno='$usuario' AND idencuesta='$encuesta' AND pregunta='$pregunta'";
		$rs = $con->prepare($sql);
		$rs->execute();
		$row = $rs->fetch();
		if ($row['cuenta']==0) 
...
<span class="foo"> Lee detenidamente los siguientes enunciados. Elije s&oacute;lo una respuesta, si te es dif&iacute;cil identificarte con alguna, selecciona aquella que 
m&aacute;s se asemeje a tu forma de pensar; trata de ser honesto.<br> <br>
La informaci&oacute;n que proporciones es confidencial y en resguardo del Departamento de Mejoramiento Acad&eacute;mico. 
Al finalizar el inventario podr&aacute;s consultar tus resultados y contactarnos para establecer  
estrategias para mejorar en tu vida personal y acad&eacute;mica. 
<!--</td>		
		</tr>
	</table>
</div>-->
...
<b align="center"> Servicios disponibles:</b> </p> 
     ...
     <LI align="left"><A HREF="datos-personales.php">Capturar/Modificar datos personales</A></LI>
	    ...
	    <!--- CAMBIO: INICIO --->
		<!--<LI align="left"><A HREF="mensaje-temporal.php">Revisar mis indicadores</A></LI>-->
	<!--- CAMBIO: FIN --->
            <?php else: ?>		
